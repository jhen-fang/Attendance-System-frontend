...
export const getTaiwanHolidays = async (): Promise<Date[]> => {
  ...
  const holidays = raw
    ...
    .map((item: any) => new Date(`${item.date.slice(0, 4)}-${item.date.slice(4, 6)}-${item.date.slice(6, 8)}`));

  const pastDates = Array.from({ length: 365 }, (_, i) => dayjs().subtract(i + 1, 'day').toDate());
